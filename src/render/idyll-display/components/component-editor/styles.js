...
export default withContext(
  class Styles extends React.PureComponent {
    ...
    getStyles(props) {
      ...
      const _styles = eval(
        `(function() { return ${_stylesProp.value || ''} })()`
      );
      let cssString =
        postcss().process(_styles, { parser: postcssJs }).css || '';
    ...
    onExecute(cssString) {
      ...
      if (targetNode.properties.style) {
        targetNode.properties.style.value = JSON.stringify(
      ...
      return (
        <div className={'idyll-code-editor'}>
          ...
          <div
            ...
            <div>
              <a style={{ color: '#ccc', textDecoration: 'underline' }}>
                CSS Syntax
              </a>
            </div>
          </div>
